...
impl Assembler {
    pub fn new(origin: u16) -> Self { Self { origin } }
    ...
    pub fn assemble(&self, source: &str) -> Result<AssemblyOutput, String> {
        ...
        for (lineno, tokens) in &lines {
            if tokens.is_empty() { continue; }
            if tokens.first().map_or(false, |t| t.ends_with(':')) { continue; }
    ...
    fn tokenize<'a>(&self, source: &'a str) -> Vec<(usize, Vec<String>)> {
        source.lines().enumerate().filter_map(|(i, line)| {
            ...
            let tokens: Vec<String> = line
                ...
                .collect();
            if tokens.is_empty() { None } else { Some((i + 1, tokens)) }
        }).collect()
    }
    ...
    fn emit(&self, tokens: &[String], syms: &HashMap<String, u16>, lineno: usize) -> Result<Vec<u16>, String> {
        let mnemonic = tokens[0].as_str();

        macro_rules! reg {
            ($t:expr) => { parse_reg($t, lineno)? }
        }
        macro_rules! resolve {
            ($t:expr) => { resolve_imm($t, syms, lineno)? }
        }
        ...
        let words = match mnemonic {
            ...
            "ADDI"  => vec![encode_ri(Opcode::Addi, reg!(&tokens[1]), resolve!(&tokens[2]))],
            ...
            "SHL"   => vec![encode_ri(Opcode::Shl, reg!(&tokens[1]), resolve!(&tokens[2]))],
            "SHR"   => vec![encode_ri(Opcode::Shr, reg!(&tokens[1]), resolve!(&tokens[2]))],

            "CMP"   => vec![encode_rr(Opcode::Cmp,  reg!(&tokens[1]), reg!(&tokens[2]))],

            "JMP"   => { let a = resolve!(&tokens[1]); vec![encode_bare(Opcode::Jmp), a] }
            "JZ"    => { let a = resolve!(&tokens[1]); vec![encode_bare(Opcode::Jz),  a] }
            "JNZ"   => { let a = resolve!(&tokens[1]); vec![encode_bare(Opcode::Jnz), a] }
            "JC"    => { let a = resolve!(&tokens[1]); vec![encode_bare(Opcode::Jc),  a] }
            "JN"    => { let a = resolve!(&tokens[1]); vec![encode_bare(Opcode::Jn),  a] }
            "CALL"  => { let a = resolve!(&tokens[1]); vec![encode_bare(Opcode::Call), a] }
            ...
            "DW"    => tokens[1..].iter().map(|t| resolve_imm(t, syms, lineno)).collect::<Result<Vec<_>, _>>()?,
...
fn encode_bare(op: Opcode) -> u16 { (op as u16) << 10 }
...
fn resolve_imm_simple(s: &str) -> u16 {
    ...
    if let Ok(v) = s.parse::<u16>() { return v; }
    if let Ok(v) = s.parse::<i16>() { return v as u16; }
    0
}
...
fn resolve_imm(s: &str, syms: &HashMap<String, u16>, lineno: usize) -> Result<u16, String> {
    ...
    Err(format!("Line {}: Cannot resolve '{}' as immediate or label", lineno, s))
}
...
pub mod cli {
    ...
    pub fn run(src_path: &Path, out_path: &Path) -> Result<(), String> {
        ...
        println!("Assembled {} words ({} bytes) → {}", output.words.len(), bytes.len(), out_path.display());
        println!("Symbols:");
